...
CLOTH_TEX_PATH = "./texel_checker.png"
SPHERE_TEX_PATH = "./db.png"  # if missing, we fallback to CLOTH_TEX_PATH
...
class App:
    def __init__(self):
        ...
        def load_texture_2d(path: str, fallback_path: str) -> wgpu.GPUTexture:
            try:
            ...
            self.device.queue.write_texture(
                ...
                data=np.asarray(img),
                data_layout={"bytes_per_row": img.size[0] * 4},
                size=texture_size,
            )
            return tex

        # Two different textures
        self.tex_cloth = load_texture_2d(CLOTH_TEX_PATH, CLOTH_TEX_PATH)
        self.tex_sphere = load_texture_2d(SPHERE_TEX_PATH, CLOTH_TEX_PATH)
        ...
        # ---- cloth geometry (render mesh) ----
        self.cloth_w = 80
        self.cloth_h = 60
        self.spacing = 0.05
        self.nverts = self.cloth_w * self.cloth_h

        vtx0, idx0 = cloth_grid(self.cloth_w, self.cloth_h, self.spacing, y0=1.2)
        self.cloth_vertex_stride = 8 * 4  # pos3 + nrm3 + uv2

        self.cloth_vertex_buffer = self.device.create_buffer_with_data(
            data=vtx0,
            usage=wgpu.BufferUsage.VERTEX | wgpu.BufferUsage.STORAGE | wgpu.BufferUsage.COPY_DST,
        )
        self.cloth_index_buffer = self.device.create_buffer_with_data(
            data=idx0, usage=wgpu.BufferUsage.INDEX
        )
        self.cloth_index_count = idx0.size

        # Wireframe indices for cloth
        wire_idx = make_cloth_wire_indices(self.cloth_w, self.cloth_h)
        self.cloth_wire_index_buffer = self.device.create_buffer_with_data(
            data=wire_idx, usage=wgpu.BufferUsage.INDEX
        )
        self.cloth_wire_index_count = wire_idx.size

        # ---- simulation state (ping-pong) ----
        pos0, vel0 = make_initial_state(self.cloth_w, self.cloth_h, self.spacing, y0=1.2, mass=0.25)
        self.pos_a = self.device.create_buffer_with_data(
            data=pos0, usage=wgpu.BufferUsage.STORAGE | wgpu.BufferUsage.COPY_DST
        )
        self.vel_a = self.device.create_buffer_with_data(
            data=vel0, usage=wgpu.BufferUsage.STORAGE | wgpu.BufferUsage.COPY_DST
        )
        self.pos_b = self.device.create_buffer(size=self.pos_a.size, usage=wgpu.BufferUsage.STORAGE)
        self.vel_b = self.device.create_buffer(size=self.vel_a.size, usage=wgpu.BufferUsage.STORAGE)
        ...
        # Sphere template at radius=1 centered at origin (so we can scale positions)
        self._sphere_template_vtx, sph_idx = sphere(48, 24, center=(0.0, 0.0, 0.0), radius=1.0)
        sph_vtx = self._sphere_template_vtx.copy()
        sph_vtx[:, 0:3] = self.sphere_center[None, :] + self.sphere_radius * self._sphere_template_vtx[:, 0:3]

        self._sphere_radius_gpu = float(self.sphere_radius)
        self.sphere_vertex_buffer = self.device.create_buffer_with_data(
            data=sph_vtx, usage=wgpu.BufferUsage.VERTEX | wgpu.BufferUsage.COPY_DST
        )
        self.sphere_index_buffer = self.device.create_buffer_with_data(
            data=sph_idx, usage=wgpu.BufferUsage.INDEX
        )
        self.sphere_index_count = sph_idx.size

        # ---- compute: sim params ----
        # Layout matches SimParams in cloth_compute.wgsl: 20 floats + 4 u32 = 96 bytes.
        self.sim_params_buffer = self.device.create_buffer(
            size=96, usage=wgpu.BufferUsage.UNIFORM | wgpu.BufferUsage.COPY_DST
        )

        compute_bg_layout = self.device.create_bind_group_layout(
            entries=[
                {
                    "binding": 0,
        ...
        compute_layout = self.device.create_pipeline_layout(bind_group_layouts=[compute_bg_layout])

        with open("./cloth_compute.wgsl", "r") as f:
            cshader = self.device.create_shader_module(code=f.read())

        self.pipe_step = self.device.create_compute_pipeline(
            layout=compute_layout, compute={"module": cshader, "entry_point": "step"}
        )
        self.pipe_write_vtx = self.device.create_compute_pipeline(
            layout=compute_layout, compute={"module": cshader, "entry_point": "write_vertices"}
        )
                ...
                entries=[
                    {
                        ...
                        "resource": {
                            ...
                            "offset": 0,
                            "size": self.sim_params_buffer.size,
                        },
                    },
                    ...
                    {"binding": 4, "resource": {"buffer": vel_out, "offset": 0, "size": vel_out.size}},
                    {
                        "binding": 5,
                        "resource": {
                            "buffer": self.cloth_vertex_buffer,
                            "offset": 0,
                            "size": self.cloth_vertex_buffer.size,
                        },
                    },
                ],
            )
...
# ---- simulation tuning ----
    ...
    def reset_all(self):
        ...
        # Update sphere mesh immediately
        sph_vtx = self._sphere_template_vtx.copy()
        sph_vtx[:, 0:3] = self.sphere_center[None, :] + self.sphere_radius * self._sphere_template_vtx[:, 0:3]
        self.device.queue.write_buffer(self.sphere_vertex_buffer, 0, sph_vtx.tobytes())

        # Reset cloth sim buffers (pos/vel) in BOTH ping-pong buffers
        pos0, vel0 = make_initial_state(
            self.cloth_w, self.cloth_h, self.spacing, y0=self._default_y0, mass=self._default_mass
        )
        ...
        # Reset cloth vertex buffer immediately (visual reset)
        self.device.queue.write_buffer(self.cloth_vertex_buffer, 0, self._cloth_vtx0.tobytes())
    ...
    def loop(self):
        ...
        # Update sphere mesh if radius changed
        if self.draw_sphere and abs(self.sphere_radius - self._sphere_radius_gpu) > 1e-6:
            sph_vtx = self._sphere_template_vtx.copy()
            sph_vtx[:, 0:3] = self.sphere_center[None, :] + self.sphere_radius * self._sphere_template_vtx[:, 0:3]
            self.device.queue.write_buffer(self.sphere_vertex_buffer, 0, sph_vtx.tobytes())
            self._sphere_radius_gpu = float(self.sphere_radius)
